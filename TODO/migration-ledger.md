...
| 2026-08-30 | 8/PR-K9 | retention/drop runbook（owner 双签）+ ADR-0005/`core-execution-event.md` + evidence 重冻结 | 本地 evidence generator + 全量 `bash scripts/release_gate.sh`（fresh PG） | release gate `steps=29 skipped=1 failed=0`（HEAD `ff0677cd`）；pass manifests unattested 待 CI；Mongo drop 待 owner | `tmp/gate-logs/gate-summary.json`；`TODO/retention/mongo_state_store_retirement.md` |
| 2026-08-30 | 9/候选1 | core ingress 收口为 `routers/runtime_transport.py`（7 公开名）：L1 wire 基线 → L2 execution 折叠 → L3 legacy 复用（字节不变）→ L4 独立删除重复 helper/`_request_context.py` | 每批 core/BFF `ruff`+`pytest`+security scan+architecture gate；最终全量 release gate | core 383 passed、BFF 全绿；旧 helper grep 零命中；gate `steps=29 skipped=1 failed=0`（HEAD `773a4974`）；候选 2–5 留 owner/触发条件 | plan Step 9 状态块；commits `6161b4b4..773a4974` |
| 2026-08-30 | 10 | evidence retention 策略/脚本/index；三份历史 TODO 标 Superseded；收口报告与 FINAL runbook；LOC/C901/F401/SDK 统计 | `scripts/evidence_retention.py`、`scripts/architecture_gate.py --mode report`、全量 release gate（非 FINAL） | index：current 145/historical 14；非 FINAL gate `steps=29 skipped=1 failed=0`；releasable 待 CI attest | `TODO/Step10_收口报告与FINAL_runbook.md`；`tmp/acceptance/index.json`；`tmp/gate-logs/step10-architecture-report.json` |
