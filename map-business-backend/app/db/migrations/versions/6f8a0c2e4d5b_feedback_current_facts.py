...
def upgrade() -> None:
    op.add_column('message_feedback', sa.Column('conversation_id', sa.dialects.postgresql.UUID(as_uuid=True), nullable=True), schema='map_control')
    op.add_column('message_feedback', sa.Column('request_id', sa.String(length=128), nullable=True), schema='map_control')
    op.add_column('message_feedback', sa.Column('user_id', sa.String(length=128), nullable=True), schema='map_control')
    op.add_column('message_feedback', sa.Column('rating', sa.String(length=16), nullable=True), schema='map_control')
    op.add_column('message_feedback', sa.Column('reason_codes', sa.dialects.postgresql.JSONB(), nullable=True), schema='map_control')
    op.add_column('message_feedback', sa.Column('reason_other', sa.Text(), nullable=True), schema='map_control')
    op.add_column('message_feedback', sa.Column('correction_text', sa.Text(), nullable=True), schema='map_control')
    op.add_column('message_feedback', sa.Column('status', sa.String(length=16), server_default='open', nullable=False), schema='map_control')
    op.add_column('message_feedback', sa.Column('version', sa.Integer(), server_default='1', nullable=False), schema='map_control')
    op.add_column('message_feedback', sa.Column('withdrawn_at', sa.DateTime(timezone=True), nullable=True), schema='map_control')
    ...
    op.alter_column('message_feedback', 'kind', existing_type=sa.String(length=16), nullable=True, schema='map_control')
    op.alter_column('message_feedback', 'reason', existing_type=sa.Text(), nullable=True, schema='map_control')
    ...
    op.execute(
        sa.text(
            ...
            "rating = CASE kind WHEN 'thumbs_up' THEN 'helpful' WHEN 'thumbs_down' THEN 'unhelpful' END, "
            "reason_other = reason "
...
def downgrade() -> None:
    op.drop_index('ix_feedback_workspace_created', table_name='message_feedback', schema='map_control')
    op.drop_index('uq_feedback_active_message_user', table_name='message_feedback', schema='map_control')
