...
| ML-04 | legacy/AgentScope 双引擎、`MAP_AGENT_ENGINE`、`RuntimeAgent` seam、`_compat_session` | Step 5 / PR-H | 待认领 | 跟踪中 |
| ML-05 | `LLMEngine` 宽接口、同义命名族、sync path | Step 6 / PR-I | 待认领 | 跟踪中 |
| ML-06 | 文件快照 `admin_state.json` 主路径、startup reconciler、generic AdminState 更新 | Step 7 / PR-J | 待认领 | 跟踪中 |
| ML-07 | Mongo `state_store`、Webhook handler、`fire_and_forget`、telemetry collections | Step 8 / PR-K | 待认领 | 跟踪中 |
