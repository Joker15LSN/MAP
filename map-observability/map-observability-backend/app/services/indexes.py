...
def ensure_indexes(
    database: Database, collections: MongoCollections, ignore_auth_error: bool = True
) -> bool:
    ...
    try:
        ...
        if ignore_auth_error and _is_auth_error(exc):
            logger.warning(
                "Skipping MongoDB index creation because credentials do not have "
                "createIndexes privilege. Set INDEX_ENSURE_MODE=skip to silence "
                "this warning."
            )
            return False
        raise
