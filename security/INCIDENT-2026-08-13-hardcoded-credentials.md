...
- **已移除**：仓库内全部固定凭据（gpustack 系列 token、内网模型端点、
  硬编码 API key / 数据库口令）。生产代码一律改为环境变量注入，未配置
  时下游 fail-closed。
...
| 5 | Milvus `root` / `"password"` 固定口令 | `wenshu_agent.py`（生产路径）、`_wenshu_split_question/split_question.py`（测试函数） | 改 `MAP_MILVUS_USER` / `MAP_MILVUS_PASSWORD` env | PENDING-REVOCATION | security owner |
...
- 自动扫描门禁：`map_core/tests/test_hardcoded_credential_scan.py` 在 CI
  （pytest 全量）中扫描 map_core 源码树，出现 gpustack/AWS/GitHub/OpenAI
  等固定凭据模式即失败。
